...
#define	DEBCEINIT // mod - 09/02/05,mark - 07/27/05,add - 07/25/05,bolo for the debug messages shown on the console 
// #define	DEBCEPC // add - 07/27/05,bolo

// ++ mod - 09/06/05,bolo
#define VENDOR_ID_INFINEON  0x7a6
#define VENDOR_ID_851X      VENDOR_ID_INFINEON
#define PRODUCT_ID_8515     0x8515
#define PRODUCT_ID_8513     0x8513
#define PRODUCT_ID_8511     0x8511
// --
#define DRIVER_ID        L"LANUSB" // mod - 08/17/05,bolo
#define Registory_key	 L"Drivers\\USB\\ClientDrivers\\LANUSB" // mod - 08/17/05,bolo
...
/*
 These registry key values are existed in the file "common.reg" which is located
 at the directory listed below:

 "\WINCE500\PBWorkspaces\[PROJECT_NAME]\RelDir\[PLATFORM]_Release" OR
 "\WINCE420\PUBLIC\[PROJECT_NAME]\RelDir\[PLATFORM]_Release

 You can modify these registry key values in this "common.reg" to meet your requirement.
 It is not necessary to keep these registry key values in these variables stated below.
 Nevertheless, if you still want to restore these registry key values in these variables,
 these registry key values in the "common.reg" will be replaced by these values in these
 variables. It means you should modify the values listed in these variables not in 
 "common.reg" when you want to change these registry key values. (add - 08/26/05,bolo)
*/

// Values for [HKEY_LOCAL_MACHINE\Comm\LANNDS] and [HKEY_LOCAL_MACHINE\Comm\LANNDS1]
REG_VALUE_DESCR CommKeyValues[] =
{
    /* mark - 09/06/05,bolo; you can recover this line when the device name is disappeared on "Network and Dial-up Connections".
    (TEXT("DisplayName")), REG_SZ, (PBYTE)(TEXT("851X Series USB-to-LAN Adapter")), // mod - 09/02/05, 08/17/05,bolo
    */
...
// Values for [HKEY_LOCAL_MACHINE\Comm\LANNDS\Linkage]
REG_VALUE_DESCR LinkageKeyValues[] =
{
    // unmark - 09/02/05, mark - 08/26/05,bolo
    (TEXT("Route")), REG_MULTI_SZ, (PBYTE)(TEXT("LANNDS1\0")),
    //
    NULL,            0,            NULL
};

// Values for [HKEY_LOCAL_MACHINE\Comm\LANNDS1\Parms]
REG_VALUE_DESCR ParmKeyValues[] =
{

    (TEXT("BusNumber")), REG_DWORD, (PBYTE)0,
    (TEXT("BusType")),   REG_DWORD, (PBYTE)(0), // mod - 08/25/05,bolo; org: (PBYTE)(-1)
    /* mark - 09/02/05,08/26/05,bolo
    (TEXT("MediaType")), REG_DWORD, (PBYTE)(0), // add - 08/25/05,bolo; 0:AutoSense, 1:10BaseT, 2:10BaseT Full_Duplex, 3:100BaseTX, 4:100BaseTX Full_Duplex
    (TEXT("MediaSel")),  REG_DWORD, (PBYTE)(LAN_MEDIA_SELECT), // add - 08/25/05,bolo; 0:AutoSense, 1:Ethernet, 2:HomeLAN, 3:MAC-MII
    */
    NULL,                0,         NULL
};
...
// Values for [HKEY_LOCAL_MACHINE\Comm\LANNDS1\Parms\Tcpip]
...
//const LPCWSTR Registory_key = {TEXT("Drivers\\USB\\ClientDrivers\\LANUSB")};
//#define Registory_key L"Drivers\\USB\\ClientDrivers\\SMC_USBLAN_Driver"
// mod - 08/17/05,bolo
LPWSTR KeyNames[] =
{
    // unmark - 09/02/05, mark - 08/26/05,bolo
...
DWORD          RegVendorID, RegProductID; // add - 09/06/05,bolo
...
BOOL DllEntry( HANDLE hDllHandle, DWORD dwReason, LPVOID lpreserved )
{
    switch (dwReason)
    {
        case DLL_PROCESS_ATTACH:
            ...
            #ifdef DEBCEPC // add - 07/27/05,bolo
            NKDbgPrintfW(TEXT("DllEntry: DLL_PROCESS_ATTACH (LANUSB)!\n\r"));
            #endif
        ...
        case DLL_PROCESS_DETACH:
        	#ifdef DEBCEPC // add - 07/27/05,bolo
            NKDbgPrintfW(TEXT("DllEntry: DLL_PROCESS_DETACH (LANUSB)!\n\r"));
            #endif
            #ifdef LANUSB_DEBUG_PC
...
BOOL USBInstallDriver( LPCWSTR szDriverLibFile )  // @parm [IN] - Contains client driver DLL name
{
    ...
    DWORD       i; // add - 09/06/05,bolo
    DWORD       VendorID[]  = {VENDOR_ID_INFINEON, VENDOR_ID_INFINEON, VENDOR_ID_INFINEON}; // add - 09/06/05,bolo
    DWORD       ProductID[] = {PRODUCT_ID_8515, PRODUCT_ID_8513, PRODUCT_ID_8511}; // add - 09/06/05,bolo
...
#ifdef DEBCEINIT // add - 07/27/05,bolo
    // unmark - 09/05/05,bolo
    NKDbgPrintfW(TEXT("*** USBInstallDriver ***\n\r"));
#endif
...
#ifdef DEBCEINIT // add - 07/27/05,bolo
    // mod - 09/06/05,bolo
    NKDbgPrintfW(TEXT("USBInstallDriver: pRegisterId\n\r"));
#endif
   
            for (i = 0; i < (sizeof(VendorID)/sizeof(DWORD)); i++) { // add - 09/06/05,bolo
                ...
                DriverSettings.dwVendorId      = VendorID[i];   // mod - 09/06/05,bolo; org: VENDOR_ID;
                DriverSettings.dwProductId     = ProductID[i];  // mod - 09/06/05,bolo; org: PRODUCT_ID;
...
#ifdef DEBCEINIT // add - 07/27/05,bolo
    // mod - 09/06/05, bolo
    NKDbgPrintfW(TEXT("USBInstallDriver: pRegisterSettings, VID = 0x%x, PID = 0x%x\n\r"),DriverSettings.dwVendorId,DriverSettings.dwProductId);
#endif
...
#ifdef DEBCEINIT // add - 07/27/05,bolo
    // unmark - 09/05/05,bolo
    NKDbgPrintfW(TEXT("USBInstallDriver: pUnRegisterId\n\r"));
#endif                  	
...
BOOL USBUnInstallDriver()
{
...
#ifdef DEBCEINIT // add - 07/27/05,bolo
    // mod - 09/06/05,bolo	
    NKDbgPrintfW(TEXT("USBUnInstallDriver: VID = 0x%x, PID = 0x%x\n\r"),RegVendorID,RegProductID);
#endif
    ...
    if(hInst)
    {
        ...
        if(pUnRegisterSettings)
        {
            ...
            DriverSettings.dwVendorId      = RegVendorID;   // mod - 09/06/05,bolo; org: VENDOR_ID;
            DriverSettings.dwProductId     = RegProductID;  // mod - 09/06/05,bolo; org: PRODUCT_ID;
            DriverSettings.dwReleaseNumber = USB_NO_INFO;
...
#ifdef DEBCEINIT // add - 09/06/05, bolo
    NKDbgPrintfW(TEXT("USBUnInstallDriver: pUnRegisterSettings, VID = 0x%x, PID = 0x%x\n\r"),DriverSettings.dwVendorId,DriverSettings.dwProductId);
#endif
...
//    LPUnPlugNotify         pNdsUnplugN;
...
#ifdef DEBCEINIT // add - 07/27/05,bolo
    // unmark - 09/05/05,bolo    
    NKDbgPrintfW(TEXT("*** USBDeviceNotifications ***\n\r"));
#endif
    ...
    switch(dwCode)
    {
        case USB_CLOSE_DEVICE:
       		...
       		#ifdef DEBCEINIT // add - 07/27/05,bolo
                // unmark - 09/05/05,bolo
            	NKDbgPrintfW(TEXT("*** USB_CLOSE_DEVICE ***\n\r"));
            #endif
...
//    LPAttachNotify        pAttachN;
    ...
    *fAcceptControl = FALSE;
	...
	#ifdef DEBCEINIT // add - 07/27/05,bolo
    // unmark - 09/05/05,bolo
  	NKDbgPrintfW(TEXT("USBDeviceAttach: VID = 0x%x, PID = 0x%x\n\r"), lpDriverSettings->dwVendorId, lpDriverSettings->dwProductId); // add - 07/27/05,bolo
	#endif	
    ...
    RegVendorID  = lpDriverSettings->dwVendorId;   // ++ add - 09/06/05,bolo
    RegProductID = lpDriverSettings->dwProductId;  // --
    
	Lock();
    
  //if( lpDriverSettings->dwVendorId == VENDOR_ID && lpDriverSettings->dwProductId == PRODUCT_ID )
    if( lpDriverSettings->dwVendorId == VENDOR_ID_851X ) // mod - 09/06/05,bolo; org: VENDOR_ID
    {
#ifdef DEBCEINIT // add - 07/27/05,bolo
    // mod - 09/06/05,bolo
   	NKDbgPrintfW(TEXT("USBDeviceAttach: 851X -> VID = 0x%x, PID = 0x%x\n\r"), RegVendorID, RegProductID); // add - 07/27/05,bolo
#endif
    // #ifdef LANUSB_DEBUG
